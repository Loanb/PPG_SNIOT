...
static int max30102_start_acquisition(const struct device *dev)
{
    struct max30102_data *data = dev->data;
    const struct max30102_config *config = dev->config;

    /* Write the intended mode (from devicetree) to the mode register */
    if (i2c_reg_write_byte(data->i2c, config->i2c_addr,
                           MAX30102_REG_MODE_CFG,
                           data->intended_mode))
    {
        LOG_ERR("Failed to start acquisition");
        return -EIO;
    }

    LOG_DBG("MAX30102 acquisition started (mode 0x%02x)", data->intended_mode);
    return 0;
}

static int max30102_stop_acquisition(const struct device *dev)
{
    struct max30102_data *data = dev->data;
    const struct max30102_config *config = dev->config;

    /* Write shutdown mode (0x00) to the mode register */
    if (i2c_reg_write_byte(data->i2c, config->i2c_addr,
                           MAX30102_REG_MODE_CFG,
                           0x00))
    {
        LOG_ERR("Failed to stop acquisition");
        return -EIO;
    }

    LOG_DBG("MAX30102 acquisition stopped");
    return 0;
}
...
static int max30102_attr_set(const struct device *dev,
                             ...
                             const struct sensor_value *val)
{
    ...
    switch ((int)attr)
    {
    case MAX30102_ATTR_LED_PA:
        // Pulse amplitude is an 8-bit integer value (0-255)
        if (val->val2 != 0 || val->val1 < 0 || val->val1 > 255)
        {
            LOG_ERR("Invalid pulse amplitude value. Must be 0-255.");
            return -EINVAL;
        }

        uint8_t pa_val = (uint8_t)val->val1;

        if (chan == SENSOR_CHAN_RED)
        {
            reg_addr = MAX30102_REG_LED1_PA;
            LOG_DBG("Setting RED LED PA (Reg 0x%02X) to %d", reg_addr, pa_val);
        }
        else if (chan == SENSOR_CHAN_IR)
        {
            reg_addr = MAX30102_REG_LED2_PA;
            LOG_DBG("Setting IR LED PA (Reg 0x%02X) to %d", reg_addr, pa_val);
        }
        else
        {
            LOG_ERR("attr_set not supported for channel %d", chan);
            return -ENOTSUP;
        }

        if (i2c_reg_write_byte(data->i2c, config->i2c_addr, reg_addr, pa_val))
        {
            LOG_ERR("Failed to set pulse amplitude for chan %d", chan);
            return -EIO;
        }
        break;
    case MAX30102_ATTR_ACQUISITION:
        if (val->val1 == 1)
        {
            return max30102_start_acquisition(dev);
        }
        else if (val->val1 == 0)
        {
            return max30102_stop_acquisition(dev);
        }

        LOG_ERR("Invalid value for ACQUISITION attr (1=start, 0=stop)");
        return -EINVAL;
        break;
    default:
...
static int max30102_init(const struct device *dev)
{
    ...
    // Save acquisition mode
    data->intended_mode = config->mode;
    // Put device in shutdown
    if (i2c_reg_write_byte(data->i2c, config->i2c_addr, MAX30102_REG_MODE_CFG, 0x00))
    {
        LOG_ERR("Failed to set initial shutdown mode");
        return -EIO;
    }
...
#define MAX30102_DEFINE(inst)                                                    \
    ...
    static struct max30102_config max30102_config_##inst = {                     \
        .i2c_label = DT_PROP(DT_BUS(DT_DRV_INST(inst)), label),                  \
